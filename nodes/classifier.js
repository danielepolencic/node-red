...
function createDataset(entries) {
    const texts = entries
        ...
        .reduce((obj, it) => {
        return Object.assign(Object.assign({}, obj), { [`row-${it.gs$cell.row}`]: it.content.$t });
    }, {});
    ...
    categoriesSet.forEach((category) => {
        dataset.add(category, data.filter((it) => it.category === category).map((it) => it.doc));
    });
    return dataset;
}
function tokenize(str) {
    const tree = new English().parse(str);
    const file = { data: { keywords: [], keyphrases: [] } };
    pos(tree);
    keywords(tree, file);
